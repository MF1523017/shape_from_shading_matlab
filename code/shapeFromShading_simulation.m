function [k_, surface_normals] = shapeFromShading_simulation(lights, imageLocation, imageExtension, imageName, imageCount, imageHeight, imageWidth, isGrayScale )
...
% for i=1:m
%     for j=1:n
%         Z(i,j) = sum(q(1:i, j)) + sum(p(i,1:j));
%     end
% end
% Z = Z*-1;

gaussian = fspecial('gaussian',[5 5],3);
% Z = imfilter(Z,gaussian,'same');
k_ = imfilter(k,gaussian,'same');
surface_normals=cat(3,p,q,k);
end
